...
| 그림 | 무엇을 주장하는가 | 형식 | 용도 |
|---|---|---|---|
| 1. As-Is → To-Be | 직접 이어져 있던 연결을 한 지점으로 모았다 | SVG | 제출 이미지 |
...
과제 2.1 이 준 시나리오 그림을 그대로 다시 그린 것이 As-Is 다. 연결선이 **전부 시스템끼리 직접 이어져 있다.**
주문자가 회계 영수증 관리자의 FTP 주소도, 쇼핑몰 주문 DB 접속정보도 알아야 하고, 형식 변환도 보내는 쪽 책임이다.
