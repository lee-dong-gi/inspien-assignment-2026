...
| B10 | **대상 환경의 쓰기 권한** | **append-only** — DB `SELECT`/`INSERT`/`UPDATE` 만 (`DELETE` 시 `ORA-01031`), FTP `STOR` 만 (`RNFR` 시 `451`, `DELE` 시 `550 Operation not permitted`) |
...
| `FAIL` + 2xxx/3xxx 재시도 가능 | **503** | 대상 시스템 일시 장애 — 잠시 후 동일 요청 재전송 |
| `FAIL` + 2xxx/3xxx 재시도 불가 | **502** | 대상 시스템에서 확정 실패 |
| `FAIL` + `EAI-4xxx` | **500** | 우리 쪽 문제 — 송신 측이 할 수 있는 일이 없다 |
...
| `EAI-3002` | FTP_UPLOAD_ERROR (준비 단계 — **재시도 가능**, DB 가 함께 롤백된다) |
| `EAI-3004` | FTP_ENCODING_ERROR (파일명 또는 내용 인코딩 손상 — 재시도 불가) |
| `EAI-3005` | FTP_COMMIT_FAILED (**확정 단계** 업로드 실패 — 재시도 불가, 수동 조치) |
| `EAI-4001` | BATCH_LOCK_ACQUIRE_FAILED |
| `EAI-4002` | ID_ISSUE_FAILED (채번 실패 — 재시도 가능) |
| `EAI-4003` | ID_SPACE_EXHAUSTED (26,000 소진 — 재시도 불가) |
| `EAI-4004` | DELIVERY_ERROR (전달 조율 실패 — 수신처 미등록 등 **우리 쪽** 문제) |
| `EAI-4005` | FLOW_ERROR (파이프라인 실행 오류 — 구간에 귀속되지 않는 예외) |
...
| D-14 | 전달 조율 결과 표현 | `DeliveryCoordinator.deliver()` 반환을 `int` → **`DeliveryOutcome`** 으로 변경 | 3.9 표 4번 행(JDBC commit 성공 → FTP rename 실패)은 **예외로 표현하면 안 되는 상태**다. DB 에 63행이 유효하게 들어있는데 `FAIL` 로 응답하면 호출자가 재요청하고, 그 재요청이 곧 중복 적재다 — `EAI-3005` 를 재시도 불가로 못박은 이유와 같다. 반대로 조용히 `SUCCESS` 를 주면 영수증이 영원히 수집되지 않는다. **예외 = "아무 데도 남지 않았다(재시도 안전)", 반환 = "어딘가엔 남았다"** 로 경계를 고정했다. 3.10 응답 포맷이 이미 `PARTIAL` 을 1급 상태로 두고 있으므로, 조율자가 그 상태를 표현하지 못하면 거기서 정보가 잘린다 |
| D-15 | 보상 성공 여부 집계 | `Delivery.compensate()` 를 **반환값 없는 그대로** 유지. 조율자는 보상의 성패를 모른다 | 보상 실패는 각 `Delivery` 가 `EAI-2002` / `EAI-3003` 으로 직접 기록한다. 반환값을 받으면 집계는 가능하지만, 보상은 **이미 다른 실패를 처리하는 중**에 불리므로 그 결과로 다시 분기하는 코드는 실패 경로를 한 겹 더 복잡하게 만든다. 한계를 숨기지 않고 javadoc 에 명시 |
| D-16 | `txId` 발급 주체 | `Sender.receive(MessageHeader, I)` — **`IntegrationFlow` 가 발급**하고 Sender 는 받아서 쓴다 | 기존 `receive(I)` 는 Sender 가 헤더를 만들었다. 그러면 XML 파싱이 실패했을 때 **추적 ID 자체가 존재하지 않아** 실행 이력을 남길 수도, 응답에 `txId` 를 실을 수도 없다 — 추적하려고 만든 장치가 정작 첫 구간의 실패를 못 추적한다. `txId` 는 메시지의 속성이 아니라 **실행 단위의 식별자**이므로 실행 단위가 소유하는 것이 맞다. 발급을 맨 앞으로 당기면 **어떤 실패든 txId 를 갖는다** |
| D-17 | 수신처 구간 로그 단위 | 수신처마다 **`PREPARE` 1줄 + `COMMIT` 1줄** (조율자가 기록) | 합쳐서 1줄로 줄이면 "업로드까지는 됐는데 rename 에서 죽었다" 와 "업로드부터 실패했다" 를 구분할 수 없다. 전자는 **수동 조치**, 후자는 **재시도** 로 조치가 정반대다. 부수 효과로 이력 파일이 보상 트랜잭션의 진행 순서를 그대로 보여주는 증거가 된다 — 발표 자료에 그대로 쓸 수 있다 |
| D-18 | 응답의 `txId` 전달 경로 | **`ProcessResult` 가 `txId` 를 직접 보유**. 래퍼 타입(`FlowExecution`) 미채택 | 3.10 응답에 `txId` 가 필요한데, 그 값은 실행 중에만 MDC 에 있고 `complete()` 에서 해제된다 — 결과가 스스로 들고 나오지 않으면 컨트롤러가 얻을 경로가 없다. 래퍼 대신 필드로 둔 것은 이 값이 결과의 **부가 정보가 아니라 신원**이기 때문이다 — "63건 적재 성공" 은 어느 실행의 이야기인지 없이는 운영에서 쓸모가 없다. 호출자가 없는 배치도 로그와 결과를 잇는 고리가 필요하므로 마찬가지다 |
| D-19 | HTTP 상태 코드 매핑 | `PARTIAL` 은 **200**. `FAIL` 은 에러 코드 **대역별로 4xx/5xx 분기**. `207 Multi-Status` 미채택 | 상태 코드가 하는 실질적 역할은 **호출자의 다음 행동을 정하는 것**이다 — 400 은 데이터를 고치라는 뜻, 503 은 그대로 다시 보내라는 뜻, 500 은 우리에게 연락하라는 뜻이다. 전부 500 으로 뭉뚱그리면 송신 시스템 담당자는 자기 데이터가 잘못됐다는 사실을 영원히 모른다. `PARTIAL` 을 2xx 밖으로 내보내지 않는 이유는 두 가지다 — (1) "63건 들어가고 11건 빠졌다" 는 정보는 애초에 상태 코드에 담기지 않으므로 본문을 봐야 하고, 3.10 이 이미 `result` 를 1급 필드로 두고 있다. (2) 수동 조치가 필요한 `PARTIAL`(D-14) 에 5xx 를 주면 **호출자의 재시도 로직이 곧 중복 적재**가 된다. `207` 은 의미상 가깝지만 WebDAV 확장이라 대부분의 클라이언트가 2xx 로만 처리해 실익이 없다. 구현은 `default` 없는 exhaustive switch 라, 에러 코드를 추가하면 **매핑을 정하기 전까지 컴파일이 깨진다** |
| D-20 | 컨트롤러 빈 등록 방식 | 컨트롤러만 **컴포넌트 스캔** 허용 (다른 부품은 `@Configuration` 손 조립 유지) | `@RestController` 는 `@Component` 계열이라 스캔과 `@Bean` 등록이 **겹치면 같은 핸들러가 두 번 등록**되어 매핑 충돌로 기동이 실패한다. 손 조립의 목적이었던 테스트 용이성은 여기서 손해가 없다 — 애노테이션이 붙어 있어도 `new OrderController(flow)` 가 그대로 된다. 단 `OrderFlowConfig` 와 **동일한 `@ConditionalOnProperty`** 를 건다. 제어 평면(bootstrap · probe)에서는 파이프라인 빈이 만들어지지 않으므로 조건이 없으면 주입 대상이 없어 기동이 깨진다 |
| D-21 | FTP 보상 트랜잭션 구조 | **업로드를 준비 → 확정 단계로 이동.** `.tmp` + rename 방식 철회. `FTP_COMPENSATION_FAILED`(EAI-3003) 제거, `FTP_RENAME_FAILED` → `FTP_COMMIT_FAILED`(EAI-3005 번호 유지) | **실측이 전제를 깨뜨렸다.** 대상 서버가 rename 을 거부한다(`451 Operation not permitted`). Oracle 도 `DELETE` 권한이 없다(`ORA-01031`). 두 시스템 모두 append-only 이므로(B10), `.tmp` 방식은 **매 실행마다 반드시 실패**하고 보상(삭제) 경로도 실행할 수 없다 — 드물게 일어나는 수동 조치 상황으로 설계한 경로가 정상 경로가 된다. **보상 트랜잭션을 설계할 때 가장 먼저 물을 것은 "보상이 이 환경에서 가능한가" 이며**, 불가능하다면 보상 로직을 정교하게 짜는 것이 아니라 준비 단계의 부수효과를 없애야 한다. **결과적으로 실패 경로가 더 단순해졌다** — 3.9 표 3번 행에서 보상할 것이 사라졌고, `compensate()` 가 실패할 수 있는 지점 자체가 없어졌다. **대가**: 최종명으로 바로 올리므로 전송 중단 시 잘린 파일이 회수 불가 상태로 남는다 → 업로드 후 **크기까지** 검증해 반드시 검출한다. 또 FTP 장애를 DB 확정 이후에 알게 되나, 접속·인증·디렉터리 진입은 여전히 준비 단계에서 걸러진다 |
