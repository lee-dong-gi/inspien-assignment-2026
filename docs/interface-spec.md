...
|---|---|---|
| 형식 | `[A-Z][0-9]{3}` | 과제 명시 |
| 공간 | 26 × 1,000 = **26,000개** | 유한함을 인지 |
| 부여 단위 | **주문 라인(행) 단위** | PK 가 `(ORDER_ID, APPLICANT_KEY)` 이므로 주문 단위 부여 시 ITEM 2건에서 즉시 PK 위반. 샘플의 `A113`/`B114` 도 행 단위를 지시 |
| 구현 | Redis `INCR` 원자 채번 | 재기동·동시 요청 시 중복 방지 |
| 폴백 | DB 시퀀스 테이블 | Redis 미가용 대비 |
...
| D-03 | 채번 진행 방식 | 순차 증가 | 재현성·추적성 우위 |
